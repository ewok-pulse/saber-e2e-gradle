...
class NarrowingCodecTest {
    ...
    @Test
    fun `narrowingResolution is exposed on the codec`() {
        val codec = narrowingCodec<String>(String::class.java, "Use a different type")
        assertEquals("Use a different type", codec.narrowingResolution)
    }

    private fun <T : Any> narrowingCodec(
        type: Class<T>,
        resolution: String = "Use a supported type instead"
    ): NarrowingCodec<T> =
        object : NarrowingCodec<T> {
            override val decodedType: Class<T> = type
            override val narrowingResolution: String = resolution
