...
public interface CrossProjectModelAccess {
    ...
    /**
     * Locates the project with the given path, resolving relative paths against {@code base}.
     *
     * @param referrer The project from which the return value will be used.
     * @param base The project against which a relative {@code path} is resolved.
     * @param path A path to the requested project, either absolute or relative to {@code base}.
     */
    @Nullable
    default ProjectInternal findProject(ProjectIdentity referrer, ProjectIdentity base, String path) {
        return findProject(referrer, base.getProjectPath().absolutePath(Path.path(path)));
    }
